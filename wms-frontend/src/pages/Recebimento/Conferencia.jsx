...
import {
    Box, Typography, Paper, TextField, Button, Grid, LinearProgress,
    Card, CardContent, Divider, Alert, Chip
} from '@mui/material';
// Removidos ícones suspeitos que podem causar tela branca
import { ArrowLeft, Barcode, CheckCircle2, Box as BoxIcon, CheckCircle, XCircle, ChevronRight, AlertTriangle } from 'lucide-react';
...
const Conferencia = () => {
    ...
    // Padrão true para evitar piscar tela
    ...
    const carregarTudo = async () => {
        try {
            ...
            setExibirQtdEsperada(deveExibir === true); // Garante boleano
    ...
    const carregarDados = async () => {
        try {
            ...
            // Força um novo objeto para o React detectar mudança
            setRecebimento({ ...data });
    ...
    const handleFinalizar = async () => {
        if (!window.confirm("Deseja realmente finalizar a conferência?")) return;
        try {
            setLoading(true);
    ...
    const handleCancelar = async () => {
        if (!window.confirm("ATENÇÃO: Isso apagará TODO o progresso. Continuar?")) return;
        try {
            setLoading(true);
    ...
    return (
        <Box>
            <Button startIcon={<ArrowLeft />} onClick={() => navigate('/recebimento')} sx={{ mb: 2 }}>
                Voltar
            </Button>
                ...
                <Grid item xs={12} md={7}>
                    <Paper sx={{ p: 4, borderRadius: 2, border: '1px solid #e2e8f0' }}>
                        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                            ...
                            {!exibirQtdEsperada && (
                                <Chip label="Modo Cego Ativo" color="warning" variant="outlined" sx={{ fontWeight: 'bold' }} />
                            )}
                        </Box>
                        ...
                        <form onSubmit={handleSubmit}>
                            <Box mb={3}>
                                ...
                                <TextField
                                    ...
                                    autoComplete="off"
                                    InputProps={{ startAdornment: <Barcode size={20} style={{ marginRight: 8, opacity: 0.5 }} /> }}
                                    sx={{ '& input': { fontSize: '1.2rem', p: 1.5 } }}
                                />
                            </Box>
                            ...
                            <Box mb={4}>
                                ...
                                <TextField
                                    inputRef={qtdInputRef}
                                    fullWidth
                                    type="number"
                                    value={form.qtd}
                                    onChange={(e) => setForm({ ...form, qtd: e.target.value })}
                                    placeholder="Ex: 10"
                                    InputProps={{ startAdornment: <BoxIcon size={20} style={{ marginRight: 8, opacity: 0.5 }} /> }}
                                    sx={{ '& input': { fontSize: '1.2rem', p: 1.5 } }}
                                />
                            </Box>

                            <Button type="submit" variant="contained" fullWidth size="large" disabled={!form.sku || !form.qtd} sx={{ py: 2 }}>
                                Gerar Etiqueta LPN
                            </Button>
                        </form>
                    </Paper>
                    ...
                    {ultimoLpn && (
                        <Alert icon={<CheckCircle2 fontSize="inherit" />} severity="success" sx={{ mt: 3 }}>
                            LPN Gerado: <b>{ultimoLpn.codigo}</b> ({ultimoLpn.sku} - {ultimoLpn.qtd} un.)
                        </Alert>
                    )}
                </Grid>
                        ...
                        <CardContent>
                            ...
                            {exibirQtdEsperada ? (
                                <>
                                    <Box display="flex" justifyContent="space-between" mb={1}>
                                        ...
                                        <Typography variant="body2" fontWeight="bold">{Math.round(progresso)}%</Typography>
                                    </Box>
                            ...
                            <Box sx={{ maxHeight: 300, overflow: 'auto' }}>
                                {recebimento?.itens?.map((item) => {
                                    ...
                                    const conferiuAlgo = qtdConf > 0;
                                    const completo = exibirQtdEsperada && (qtdConf >= qtdNota);
                                    ...
                                    return (
                                        <Box
                                            ...
                                            sx={{
                                                p: 1.5, mb: 1,
                                                bgcolor: completo ? '#dcfce7' : (conferiuAlgo ? '#fffbeb' : 'white'),
                                                borderRadius: 1,
                                                border: '1px solid',
                                                borderColor: '#e2e8f0',
                                                cursor: 'pointer',
                                        ...
                                        >
                                            <Box display="flex" justifyContent="space-between" alignItems="center">
                                                ...
                                                <Box textAlign="right">
                                                    <Typography variant="caption" sx={{ bgcolor: '#f1f5f9', px: 1, borderRadius: 1, fontWeight: 'bold', display: 'block', mb: 0.5 }}>
                                                        {exibirQtdEsperada ? `${qtdConf} / ${qtdNota}` : `Qtd: ${qtdConf}`}
                                                    </Typography>
                            ...
                            <Divider sx={{ my: 3 }} />
                            <Button variant="contained" fullWidth size="large" onClick={handleFinalizar} startIcon={<CheckCircle />} sx={{ mb: 2 }}>
                                Finalizar Conferência
                            </Button>
                            <Button variant="outlined" color="error" fullWidth size="large" onClick={handleCancelar} startIcon={<XCircle />}>
            ...
            {/* MODAL DE ITENS */}
            <ItemConferenciaModal
                open={Boolean(selectedItem)}
                onClose={() => setSelectedItem(null)}
                recebimentoId={id}
                item={selectedItem}
                exibirQtdEsperada={exibirQtdEsperada}
            />
        </Box>
    );
};
